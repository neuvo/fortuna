...
GAMES = {Game.mageTheAscension: "Mage: The Ascension", Game.strikeLegion: "Strike Legion",
...
def get_name(ctx):
    return varrick.get_nick(str(ctx.message.author))
...
async def roll(ctx, *args):
    if len(args) > 0 and args[0] == 'dreidel':
        ...
        for result in results:
            await ctx.send(str(varrick.get_nick(str(ctx.message.author))) + ' rolls:')
            await ctx.send(file=discord.File(result))
    elif current_game == Game.savageWorlds:
        await ctx.send(savageWorlds.roll(str(varrick.get_nick(str(ctx.message.author))), args))
        ...
        if current_game == Game.mageTheAscension or current_game == Game.strikeLegion:
            await ctx.send(
                "%s rolls %dd%d, difficulty %d" % (varrick.get_nick(str(ctx.message.author)), num, sides, diff))
            await ctx.send(roll_dice_diff(num, sides, diff))
        elif current_game == Game.pathfinder:
            await ctx.send(
                "%s rolls %dd%d, with %d bonus" % (varrick.get_nick(str(ctx.message.author)), num, sides, diff))
        ...
        await ctx.send("%s rolls %dd6" % (varrick.get_nick(str(ctx.message.author)), num))
    ...
    if game_name in GAMES.values():
        for key in GAMES.keys():
            if GAMES[key] == game_name:
                ...
                return await client.change_presence(status=Status.online,
                                                    activity=discord.Game(name=GAMES[current_game]))
    else:
...
async def hand(ctx, *args):
    ...
    if len(args) == 1 and len(args[0]) > 0:
        ...
        await ctx.send(savageWorlds.hand(str(varrick.get_nick(str(ctx.message.author)))))
...
async def cookie(ctx, *args):
    ...
    await ctx.send('%s gets: %s' % (str(varrick.get_nick(str(ctx.message.author))), fortuneCookie.eat(numFortunes)))
